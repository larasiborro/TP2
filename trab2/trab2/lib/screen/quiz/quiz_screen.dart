...
class _QuizScreenState extends State<QuizScreen> {
  ...
  late String _question; // Guarda a questão atual
  late bool _answer; // Guarda a resposta correta
  int _score = 0; // Guarda a pontuação do jogador
  int _questionNumber = 0; // Guarda o número da questão atual
  bool _quizFinished = false; // Indica se o quiz foi concluído
  String _feedback = ''; // Feedback para o jogador após responder

  @override
  void initState() {
    super.initState();
    // Inicializa a primeira pergunta ao iniciar o quiz
    _generateQuestion();
  }

// Gera uma nova pergunta baseada no nível do quiz
  void _generateQuestion() {
    if (_quizFinished) return; // Para de gerar perguntas se o quiz terminou

    _feedback = ''; // Reinicia o feedback para cada nova pergunta

    // Gera a pergunta baseada no nível atual
    switch (widget.level) {
      case 1:
    ...
    _answer = _random
        .nextBool(); // Determina aleatoriamente se a resposta correta é "Sim" ou "Não"
...
// Gera uma pergunta para o nível 2
  ...
  void _checkAnswer(bool userAnswer) {
    ...
    // Verifica se o quiz terminou após a resposta
...
// Mostra um diálogo para avançar para o próximo nível ou finalizar o quiz
    ...
    if (widget.level == 3) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Parabéns! Você completou o nível ${widget.level}.'),
